...
pub async fn get_channel_by_id_or_handle(api_key: &str, input: &str, proxy_url: Option<&str>) -> Result<ChannelResource, Box<dyn Error>> {
    ...
    // println!("Fetching Channel URL: {}", url);
    ...
    let param: ChannelListResponse = serde_json::from_str(&text)
        .map_err(|e| format!("Failed to parse ChannelListResponse: {}. Response: {}", e, text))?;
...
pub async fn get_upload_playlist_items(api_key: &str, playlist_id: &str, max_results: u32, proxy_url: Option<&str>) -> Result<Vec<String>, Box<dyn Error>> {
    let client = build_client(proxy_url)?;
    ...
    let text = resp.text().await?;
    let list: PlaylistItemListResponse = serde_json::from_str(&text)
        .map_err(|e| format!("Failed to parse PlaylistItemListResponse: {}. Response: {}", e, text))?;
...
pub async fn get_video_details(api_key: &str, video_ids: &[String], proxy_url: Option<&str>) -> Result<Vec<VideoResource>, Box<dyn Error>> {
    if video_ids.is_empty() { return Ok(Vec::new()); }
    ...
    let list: VideoListResponse = serde_json::from_str(&text)
        .map_err(|e| format!("Failed to parse VideoListResponse: {}. Response: {}", e, text))?;
